...
function apply_move(state, log, player_slot, move_id, move_index, self_switch_target_index, hp_changed, focus_punch_pending, took_damage_this_turn, damage_taken_this_turn, rejuvenation_used_this_turn) {
  ...
  if (spec.id === "sekyps") {
    ...
    log.push({
      ...
      turn: state.turn,
      phase: spec.phaseId,
      summary: "Sekyps (curse): no end_turn causa dano flat por stack (24/48/72/...), stacka ao reaplicar, nao remove no switch e nao causa dano no turno em que o alvo troca",
      data: {
        move: spec.id,
        slot: player_slot,
        target: defender.id,
...
var MOVE_TOOLTIP_DESCRIPTIONS = {
  ...
  sekyps: "Aplica o debuff Sekyps: no end_turn causa dano flat 24 por stack (24/48/72/...), stacka ao reaplicar, nao remove no switch e nao causa dano no turno em que o alvo troca.",
...
function relay_default_switch_target(state, slot_id) {
  const player = state.players[slot_id];
  for (let index = 0;index < player.team.length; index++) {
    if (index === player.activeIndex) {
      continue;
...
function load_team_selection() {
  ...
  if (parsed && Array.isArray(parsed.selected)) {
    const filtered = parsed.selected.map((id) => canonical_monster_id(id)).filter((id) => MONSTER_BY_ID.has(id) && is_lobby_enabled_monster(id)).slice(0, 3);
...
function default_lobby_moves_for_spec(spec) {
